...
import sys
sys.path.append("./")
from models.cXVAE import cXVAE_inputEmbed
...
''' Load data '''
X1 = np.loadtxt(os.path.join(PATH_data, "TCGA",'TCGA_mRNA2_confounded_sonja.csv'), delimiter=",")
X2 = np.loadtxt(os.path.join(PATH_data, "TCGA",'TCGA_DNAm_confounded_sonja.csv'), delimiter=",")
...
conf = np.loadtxt(os.path.join(PATH_data, "TCGA",'TCGA_confounder_sonja.csv'))[:,None]
...
##             Training procedure              ##
#################################################
modelname = 'cXVAE_inputEmbed_sonja2'
maxEpochs = 150

for epoch in [1, maxEpochs]:
    # Initialize model
    model = cXVAE_inputEmbed(input_size = [X1.shape[1], X2.shape[1]],
...
for i in range(50):
    ...
    for epoch in [1, maxEpochs]:
        ...
        model = cXVAE_inputEmbed.load_from_checkpoint(ckpt_file)
