...
from models.adversarial_XVAE_multiclass_pytorch2 import XVAE, advNet, ARG_MODELTYPE
...
L.seed_everything(ARG_SEED, workers=True)
...
num_conf_regr = 2         ### None  
num_conf_clf = conf.shape[1] - num_conf_regr    ### None
...
##             Training procedure              ##
#################################################
modelname = "confounded_multi/stability/ARG_MODELTYPE/ARG_OUTPUTNAME"
ls = 50
...
for epoch in [1, epochs_ae_w_advNet]:
    ...
    model_xvae_adv = ARG_MODELTYPE(PATH_xvae_ckpt=ckpt_xvae_file,
...
for i in range(50):         
    ...
    for j in range(y_pred_all.shape[1]):
        scores[i,j] = roc_auc_score(conf_test[:,j+2], y_pred_all[:,j])
...
for i in range(50):         
    ...
    for epoch in [1, epochs_ae_w_advNet]:
        ...
        model = ARG_MODELTYPE.load_from_checkpoint(ckpt_file,
                PATH_xvae_ckpt=ckpt_xvae_file, PATH_advNet_ckpt=ckpt_advNet_file,
                map_location=torch.device('cpu'))
