...
import sys
sys.path.append("/trinity/home/skatz/PROJECTS/Multi-view-Deconfounding-VAE")
...
from models.adversarial_XVAE_multiclass import XVAE, advNet, XVAE_adversarial_multiclass ##XVAE_adversarial_multiclass


''' Set seeds for replicability  -Ensure that all operations are deterministic on GPU (if used) for reproducibility '''
np.random.seed(103294982398)
torch.manual_seed(103294982398)
L.seed_everything(103294982398, workers=True)
...
num_conf_clf = conf.shape[1]        ### None
...
##             Training procedure              ##
#################################################
modelname = test #"confounded_categ2/XVAE_advTraining/XVAE_advTraining_multiclass"
ls = 50
...
for epoch in [1, epochs_ae_w_advNet]:
    ...
    model_xvae_adv = exec(XVAE_adversarial_multiclass)(PATH_xvae_ckpt=ckpt_xvae_file,
...
for i in range(50):         
    ...
    model = exec(XVAE_adversarial_multiclass).load_from_checkpoint(ckpt_file,
