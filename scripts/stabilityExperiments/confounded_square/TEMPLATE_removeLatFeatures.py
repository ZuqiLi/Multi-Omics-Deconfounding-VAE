...
if "ARG_MODELTYPE" == "cutoff_corr_05":
    cutoff_corr = 0.5
if "ARG_MODELTYPE" == "cutoff_corr_03":
    cutoff_corr = 0.3
if "ARG_MODELTYPE" == "cutoff_pvalue":
    cutoff_pvalue = 0.05
...
conf_type = 'continuous'    ### linear, continuous, categ
...
##             Training procedure              ##
#################################################
modelname = 'confounded_square/stability/ARG_MODELTYPE/ARG_OUTPUTNAME'
#modelname = 'XVAE'
maxEpochs = 150
    ...
    for epoch in [1, maxEpochs]:
        ...
        if epoch == maxEpochs:
            ...
            for i in range(conf.shape[1]):
                ...
                elif "ARG_MODELTYPE" == "cutoff_corr_03":
                    corr = np.array([abs(pearsonr(z[:,j], conf[:,i])[0]) for j in range(z.shape[1])])
                    tmp = corr < cutoff_corr
                else:
