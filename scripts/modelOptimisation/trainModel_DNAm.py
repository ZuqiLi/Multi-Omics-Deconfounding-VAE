...
## Name of the folder
outname = "singleOmic/DNAm"
maxEpochs = 51

model = VAE(x_size=X2.shape[1],
...
x_hat= model.forward(X2_test)
x_hat = x_hat.detach().numpy()

''' Reconstruction accuracy (Pearson correlation between reconstruction and input) '''
reconAcc_x1 = reconAcc_pearsonCorr(X2_test, x_hat)
                                   

print(f'Reconstruction accuracy X2 - Pearson correlation (mean+-std)   \t {np.mean(reconAcc_x1):.3f}+-{np.std(reconAcc_x1):.3f}')
...
for i in range(20):
    ...
    z = model.generate_embedding(scale(X2)).detach().numpy()
