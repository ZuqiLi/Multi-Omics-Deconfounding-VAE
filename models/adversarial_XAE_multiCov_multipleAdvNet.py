...
class XAE_w_advNet_pingpong(L.LightningModule):
    def __init__(self, 
                 PATH_xae_ckpt,
                 PATH_advNet_ckpt,
                 dic_conf,
        ...
        self.xae_pre = XAE_preTrg.load_from_checkpoint(PATH_xae_ckpt)
    ...
    def compute_loss_combined(self, batch):
        ...
        Autoencoder loss 
        '''
        ae_loss = self.xae_pre.compute_loss(batch)
    ...
    def training_step(self, batch, batch_idx):
        if self.current_epoch % 2 == 0:
            for param in self.xae_pre.parameters():
    ...
    def test_step(self, batch, batch_idx):
        ...
        ae_loss, advNet_loss, combined_loss = self.compute_loss_combined(batch)
        x_hat = self.xae_pre.forward(*batch[:2])
        z = self.xae_pre.generate_embedding(*batch[:2])
        cov_pred = self.xae_pre.forward(*batch[:2])
    ...
    def on_test_epoch_end(self):
        ...
        clust = kmeans(LF, self.xae_pre.c)
...
class advNet(L.LightningModule):
    def __init__(self,
        ...
        ### Load pre-trained XVAE model
        self.xae = XAE_preTrg.load_from_checkpoint(PATH_xae_ckpt)
        self.xae.freeze()
    ...
    def forward(self, x1, x2):
        z = self.xae.generate_embedding(x1, x2)
...
class XAE_preTrg(L.LightningModule):
    def __init__(self, 
                 x1_size, 
                 x2_size, 
                 ls, 
